...
tinymce.PluginManager.add('media', function(editor, url) {
	...
	function sanitize(html) {
		...
		var writer = new tinymce.php.Writer(), blocked;
		...
		}, new tinymce.php.Schema({})).parse(html);

		return writer.getContent();
	}

	function updateHtml(html, data, updateAll) {
		var writer = new tinymce.php.Writer();
	...
	function createPlaceholderNode(node) {
		...
		placeHolder = new tinymce.php.Node('img', 1);
	...
	function createPreviewNode(node) {
		...
		previewWrapper = new tinymce.php.Node('span', 1);
		previewWrapper.attr({
		...
		previewNode = new tinymce.php.Node(name, 1);
		previewNode.attr({
			src: node.attr('src'),
		...
		shimNode = new tinymce.php.Node('span', 1);
	...
	editor.on('preInit', function() {
		...
		editor.serializer.addAttributeFilter('data-mce-object', function(nodes, name) {
			...
			while (i--) {
				...
				realElm = new tinymce.php.Node(realElmName, 1);
				...
				if (innerHtml) {
					innerNode = new tinymce.php.Node('#text', 3);
					innerNode.raw = true;
